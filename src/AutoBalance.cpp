...
void LoadMapSettings(Map* map)
{
    ...
    {
        LOG_DEBUG("module.AutoBalance", "LoadMapSettings: Your settings tried to set a minimum player count of {} which is greater than {}'s max player count of {}. Adjusting down.",
    ...
    LOG_DEBUG("module.AutoBalance", "LoadMapSettings: Map {} has a minimum player count of {}.", map->GetMapName(), mapABInfo->minPlayers);
...
void UpdateMapDataIfNeeded(Map* map)
{
    ...
    if (mapABInfo->configTime != lastConfigTime)
    {
        ...
        LOG_DEBUG("module.AutoBalance", "UpdateMapLevelIfNeeded(): Map {} is {}.", map->GetMapName(), mapABInfo->enabled ? "enabled" : "disabled");
           ...
           )
        {
            ...
            LOG_DEBUG("module.AutoBalance", "UpdateMapLevelIfNeeded(): Map {} scaling is disabled. Map level is now {} (original).", map->GetMapName(), mapABInfo->mapLevel);
        }
        ...
        else if (mapABInfo->avgCreatureLevel <= mapABInfo->highestPlayerLevel)
        {
            ...
            LOG_DEBUG("module.AutoBalance", "UpdateMapLevelIfNeeded(): Map {} scaling is enabled. Map level is now {} (average creature level).", map->GetMapName(), mapABInfo->mapLevel);
        }
                        ...
                        for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
                        {
                            if (Player* playerHandle = playerIteration->GetSource())
                            {
                                ...
                                if (mapPlayer && mapPlayer == player) // This is the player that entered
                                {
                                    ...
                                    case LOCALE_ruRU:
                                    {
                                        chatHandle.PSendSysMessage("|cffFF0000 [AutoBalance]|r|cffFF8000 Добро пожаловать в %s (%u-player %s). Количество игроков: %u. Установлена сложность для %u.|r",
                                            map->GetMapName(),
                                            instanceMap->GetMaxPlayers(),
                                            instanceDifficulty,
                                ...
                                else
                                {
                                    ...
                                    case LOCALE_ruRU:
                                    {
                                        chatHandle.PSendSysMessage("%s входит в подземелье. Количество игроков: %u. Установлена сложность для %u.|r",
                ...
                for (Map::PlayerList::const_iterator itr = mapPlayerList.begin(); itr != mapPlayerList.end(); ++itr)
                {
                    if (Player* mapPlayer = itr->GetSource())
                    {
                        if (mapPlayer->IsInCombat() && mapPlayer->GetMap() == map)
                        {
                            ...
                            case LOCALE_ruRU:
                            {
                                chatHandle.PSendSysMessage("|cffFF0000 [AutoBalance]|r|cffFF8000 Вы покинули подземелье во время боя. Количество игроков в экземпляре по-прежнему равно %u.", mapABInfo->playerCount);
                                break;
                            }
                            default:
                    ...
                    for (Map::PlayerList::const_iterator itr = mapPlayerList.begin(); itr != mapPlayerList.end(); ++itr)
                    {
                        if (Player* mapPlayer = itr->GetSource())
                        {
                            ...
                            if (mapPlayer != player)
                            {
                                ...
                                case LOCALE_ruRU:
                                {
                                    chatHandle.PSendSysMessage("|cffFF0000 [AutoBalance]|r|cffFF8000 %s покинул подземелье, пока шел бой. Количество игроков в экземпляре по-прежнему равно %u.", player->GetName().c_str(), mapABInfo->playerCount);
                                    break;
                                }
                                default:
                    ...
                    if (!playerList.IsEmpty())
                    {
                        for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
                        {
                            ...
                            if (mapPlayer && mapPlayer != player)
                            {
                                ...
                                case LOCALE_ruRU:
                                {
                                    chatHandle.PSendSysMessage("|cffFF0000 [AutoBalance]|r|cffFF8000 %s покинул подземелье. Количество игроков: %u. Сложность установлена для %u.|r",
                                        player->GetName().c_str(),
                                        mapABInfo->playerCount,
                                        mapABInfo->adjustedPlayerCount);
                                    break;
...
public:
    ...
    bool ResetCreatureIfNeeded(Creature* creature)
    {
        ...
        {
            // before updating the creature, we should update the map data if needed
            UpdateMapDataIfNeeded(creature->GetMap());
...
public:
    ...
    static bool HandleABSetOffsetCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!*args)
        {
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Устанавливает смещение сложности для игрока в подземелье. Пример: (Вы + offset(1) = сложность для 2 игроков).");
                break;
            }
            default:
        ...
        if (offset)
        {
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Изменение смещения сложности для игрока на %i.", offseti);
                break;
            }
            default:
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Ошибка при изменении смещения сложности для игрока! Пожалуйста, попробуйте еще раз.");
                break;
            }
            default:
    ...
    static bool HandleABGetOffsetCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        case LOCALE_ruRU:
        {
            handler->PSendSysMessage("Смещение сложности для текущего игрока = %i", PlayerCountDifficultyOffset);
            break;
        }
        default:
    ...
    static bool HandleABMapStatsCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (player->GetMap()->IsDungeon() || player->GetMap()->IsBattleground())
        {
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Карта: ID %u | %s (%u-player %s)",
                    player->GetMapId(),
                    player->GetMap()->GetMapName(),
                    player->GetMap()->ToInstanceMap()->GetMaxPlayers(),
                    player->GetMap()->ToInstanceMap()->IsHeroic() ? "Героическая" : "Нормальная",
                    mapABInfo->enabled ? "" : " | AutoBalance ОТКЛЮЧЕН");
                break;
            }
            default:
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Игроков на карте: %u (Lvl %u - %u)",
                    mapABInfo->playerCount,
            ...
            if (mapABInfo->playerCount < mapABInfo->minPlayers && !PlayerCountDifficultyOffset)
            {
                ...
                case LOCALE_ruRU:
                {
                    handler->PSendSysMessage("Скорректированное количество игроков: %u (минимум для карты)", mapABInfo->adjustedPlayerCount);
                    break;
                }
                default:
            ...
            else if (mapABInfo->playerCount < mapABInfo->minPlayers && PlayerCountDifficultyOffset)
            {
                ...
                case LOCALE_ruRU:
                {
                    handler->PSendSysMessage("Скорректированное количество игроков: %u (минимум для карты + Компенсация сложности для %u)", mapABInfo->adjustedPlayerCount, PlayerCountDifficultyOffset);
                    break;
                }
                default:
            ...
            else if (PlayerCountDifficultyOffset)
            {
                ...
                case LOCALE_ruRU:
                {
                    handler->PSendSysMessage("Скорректированное количество игроков: %u (Компенсация сложности для %u)", mapABInfo->adjustedPlayerCount, PlayerCountDifficultyOffset);
                    break;
                }
                default:
            ...
            else
            {
                ...
                case LOCALE_ruRU:
                {
                    handler->PSendSysMessage("Скорректированное количество игроков: %u", mapABInfo->adjustedPlayerCount);
                    break;
                }
                default:
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("LFG Диапазон: Lvl %u - %u (Целевое: Lvl %u)", mapABInfo->lfgMinLevel, mapABInfo->lfgMaxLevel, mapABInfo->lfgTargetLevel);
                handler->PSendSysMessage("Уровень карты: %u%s",
                    (uint8)(mapABInfo->avgCreatureLevel + 0.5f),
                    mapABInfo->isLevelScalingEnabled ? std::string("->") + std::to_string(mapABInfo->highestPlayerLevel) + std::string(" (Масштабирование уровня включено)") : std::string(" (Масштабирование уровня отключено)")
                );
                handler->PSendSysMessage("Исходный диапазон уровней существа: %u - %u (Avg: %.2f)",
                    mapABInfo->lowestCreatureLevel,
                    mapABInfo->highestCreatureLevel,
                    mapABInfo->avgCreatureLevel
                );
                handler->PSendSysMessage("Активно | Всего существ на карте: %u | %u",
                    mapABInfo->activeCreatureCount,
        ...
        else
        {
            ...
            case LOCALE_ruRU:
            {
                handler->PSendSysMessage("Цель не находится в подземелье или на поле битвы.");
                break;
            }
            default:
    ...
    static bool HandleABCreatureStatsCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        case LOCALE_ruRU:
        {
            handler->PSendSysMessage("%s (%u%s%s), %s",
                ...
                target->IsDungeonBoss() ? " | Босс" : "",
                creatureABInfo->isActive ? "Активно для статистики карты" : "Игнорируется для статистики карты");
            handler->PSendSysMessage("Множитель здоровья: %.3f", creatureABInfo->HealthMultiplier);
            handler->PSendSysMessage("Множитель маны: %.3f", creatureABInfo->ManaMultiplier);
            handler->PSendSysMessage("Множитель брони: %.3f", creatureABInfo->ArmorMultiplier);
            handler->PSendSysMessage("Множитель урона: %.3f", creatureABInfo->DamageMultiplier);
            handler->PSendSysMessage("Множитель продолжительности CC: %.3f", creatureABInfo->CCDurationMultiplier);
            handler->PSendSysMessage("Множитель опыта: %.3f  Множитель денег: %.3f", creatureABInfo->XPModifier, creatureABInfo->MoneyModifier);
            break;
        }
        default:
            handler->PSendSysMessage("%s (%u%s%s), %s",
